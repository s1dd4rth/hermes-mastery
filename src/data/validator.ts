...
// v4: drift-evidence envelope (two-source enum; git-checkout dropped per Phase 0 findings)
export interface ValidatorIntegrity {
...
function looksLikeToolNotFound(input: string): boolean {
  ...
  return (
    ...
    lower.includes('hermes-mastery is not installed') ||
    lower.includes('skill hermes-mastery not found')
  );
}
