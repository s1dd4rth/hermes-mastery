        ...
        steps: [
          {
            ...
            learn:
              '**This is the payoff.** Initialize the board, make sure the gateway is running (it dispatches the workers), and launch a swarm from a single goal. Each `--worker` is `profile:title` and is repeatable; `--verifier` and `--synthesizer` are required:\n\n```\nhermes kanban init\nhermes gateway start\nhermes kanban swarm "Write a short brief comparing three note-taking apps" \\\n  --worker researcher:"Research the options" \\\n  --verifier reviewer \\\n  --synthesizer writer\n```\n\nThis creates one durable graph: a root "blackboard" card with shared context, your worker card(s) running **in parallel**, a verifier card gated until the workers finish, and a synthesizer card that merges everything into the final result.\n\nWatch it work:\n\n```\nhermes kanban watch     # live activity\nhermes kanban list      # all cards + statuses\nhermes kanban dispatch --max 3   # kick workers now instead of waiting for the tick\n```\n\nWhen the root card reaches `done`, inspect the result: `hermes kanban show <root-id>`. You just watched your agent run a team to completion.',
            do: {
              prompt:
