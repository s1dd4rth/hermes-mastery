    ...
    <div className="flex items-center gap-1 mb-4">
      {Array.from({ length: totalSteps }, (_, i) => {
        ...
        const baseClasses = `w-10 h-10 rounded-xl flex items-center justify-center text-xs font-bold transition-all duration-300 ${
          isCurrent
            ? 'bg-hermes-accent text-white shadow-lg shadow-hermes-accent/20 scale-110'
            : isComplete
            ? 'bg-emerald-50 text-emerald-600 border border-emerald-100'
            : isLocked
            ? 'bg-hermes-bg3 text-hermes-dark/20 border border-hermes-border'
            : 'bg-hermes-bg3 text-hermes-dark/40 border border-hermes-border'
        }`;

        const interactiveClasses = isClickable
          ? ' cursor-pointer hover:scale-105 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-hermes-accent/30'
          : '';
            ...
            {i < totalSteps - 1 && (
              <div
                className={`w-8 h-[2px] transition-colors duration-500 ${
                  isComplete ? 'bg-emerald-500/30' : 'bg-hermes-border'
                }`}
              />
            )}
          </div>
