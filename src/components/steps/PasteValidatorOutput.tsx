...
type FeedbackState =
  | { kind: 'idle' }
  | { kind: 'success'; appliedCount: number; failedCount: number; manualCount: number; unknownCount: number; integrity?: ValidatorIntegrity }
    ...
    <div className="rounded-2xl border border-hermes-border bg-white shadow-sm overflow-hidden mb-4">
      ...
      >
        <div className="flex items-center gap-3">
          <ClipboardPaste size={18} className="text-hermes-accent" />
          <div>
            <div className="text-sm font-bold text-hermes-dark uppercase tracking-wider">
              Paste validator output
            </div>
            <div className="text-xs text-hermes-dark/50 font-medium mt-0.5">
              Auto-verify deterministic checks for Module {moduleNumber}
            </div>
          </div>
        </div>
        {open ? (
          <ChevronDown size={18} className="text-hermes-dark/40" />
        ) : (
          <ChevronRight size={18} className="text-hermes-dark/40" />
        )}
      </button>

      {open && (
        <div className="px-6 py-5 border-t border-hermes-border bg-hermes-bg/30 space-y-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3 flex-wrap">
              <button
                ...
                className="px-6 py-2.5 bg-hermes-accent text-white rounded-xl text-sm font-bold hover:scale-[1.02] active:scale-[0.98] disabled:opacity-30 disabled:scale-100 transition-all shadow-md shadow-hermes-accent/20"
              >
                Apply Results
              </button>
...
function LiveApiAdvanced({ open, setOpen, settings, isConfigured, update, clear }: LiveApiAdvancedProps) {
  return (
    <div className="border-t border-hermes-border pt-4 mt-2">
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-hermes-dark/50 hover:text-hermes-dark/80 transition-colors"
      >
      ...
      {open && (
        <div className="mt-3 space-y-3 bg-amber-50/40 border border-amber-200 rounded-xl p-4">
          ...
          <div>
            <label className="block text-[10px] font-bold text-hermes-dark/60 uppercase tracking-wider mb-1">
              Gateway URL
            </label>
            <input
              type="text"
          ...
          <label className="flex items-start gap-2 cursor-pointer">
            <input
              ...
              className="mt-0.5 accent-hermes-accent"
            />
            <span className="text-[11px] text-hermes-dark/80 leading-relaxed">
          ...
          {(settings.gatewayUrl || settings.token) && (
            <button
              onClick={clear}
              className="flex items-center gap-1 text-[11px] text-hermes-dark/40 hover:text-hermes-accent transition-colors"
            >
...
{`hermes skills install s1dd4rth/hermes-mastery-validator`}
    ...
    case 'live_loading':
      return (
        <div className="bg-hermes-bg3 border border-hermes-border rounded-lg p-3">
          <div className="flex items-center gap-2">
            <Loader2 size={16} className="text-hermes-dark/60 animate-spin" />
            <div className="text-xs text-hermes-dark/70 font-medium">
